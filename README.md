...
게임·음성 통화 등 지연에 민감한 통신과 대용량 다운로드가 함께 발생하는 환경에서 **실시간 트래픽을 보호하기 위해 다운로드 속도를 동적으로 조절**하는 OpenWrt 기반 프로젝트입니다.

여러 기기가 회선을 공유하면 대용량 전송으로 인해 실시간 통신의 응답이 늦어질 수 있습니다. 이를 완화하기 위해 **eBPF 기반 플로우 통계 수집, 점수 기반 트래픽 분류, 상태에 따른 속도 제어**를 구현했습니다.

실시간 트래픽과 벌크 트래픽이 동시에 관측되는 상황이 일정 시간 지속되면 벌크 클래스의 속도를 낮추고, 해당 상황이 해소되면 단계적으로 복구합니다. 현재는 이 제어 흐름을 구현한 **MVP**이며, 실제 지연 감소 효과는 대상 장비와 회선에서 검증해야 합니다.

## ✨ 주요 기능

- **플로우 통계 수집**: WAN 수신 경로에서 TC eBPF로 패킷 수, 바이트 수, 패킷 크기 분포와 활동 시간을 수집합니다.
- **점수 기반 분류**: 프로토콜, 처리량, 초당 패킷 수와 크기를 조합해 REALTIME·BULK·UNKNOWN으로 분류합니다.
- **클래스별 트래픽 처리**: 분류 결과를 BPF 정책 맵에 저장하고, 패킷을 IFB의 HTB 클래스로 연결합니다.
- **적응형 속도 제어**: 실시간·벌크 트래픽의 동시 사용이 지속되면 벌크 클래스의 전송 속도를 제한합니다.
- **단계적 복구**: 보호 유지 시간과 점진적인 속도 복구를 적용해 잦은 상태 전환을 줄입니다.
- **실행 모드 및 서비스 지원**: mock 입력, 속도 변경 dry-run, OpenWrt UCI 설정과 procd 서비스를 제공합니다.

## ⚙️ 기능 구현 및 처리 흐름

### 1. TC eBPF 기반 수신 트래픽 관찰

WAN 인터페이스의 `clsact ingress`에 eBPF 프로그램을 연결해 유입되는 패킷을 관찰합니다. 현재 파서는 **Ethernet 헤더 다음에 IPv4가 오는 TCP·UDP 패킷**을 대상으로 합니다.
...
UDP에는 실시간 점수, TCP에는 벌크 점수를 일부 부여하지만 **프로토콜만으로 분류를 확정하지 않습니다.** UDP 트래픽도 처리량과 패킷 크기 등의 조건에 따라 BULK로 분류될 수 있습니다.

각 분류는 기본 임계 점수인 6점 이상이면서 상대 점수보다 높아야 선택됩니다. 패킷 특성에 기반한 추정이므로 특정 게임이나 통화 애플리케이션을 식별하는 기능은 아닙니다.
...
HTB 우선순위는 숫자가 작을수록 높습니다. **보호 상태에서 속도를 낮추는 대상은 BULK 클래스 `1:30`**이며, 모든 클래스는 상위 클래스에 설정된 전체 속도 한도를 공유합니다.

### 4. 동시 사용 감지와 보호 상태 전환

데몬은 실시간 플로우와 벌크 플로우가 모두 관측되면 충돌 상황으로 판단합니다. 여기서 충돌은 **두 종류의 트래픽이 동시에 존재한다는 의미**이며, RTT나 큐 지연을 직접 측정해 혼잡을 판정하지는 않습니다.
...
| 큐 관리 | fq_codel — 클래스별 큐 관리 |
| 서비스 관리 | UCI, procd — OpenWrt 설정과 데몬 실행 관리 |
...
- 다운로드 방향의 트래픽을 제어하며, 업로드 셰이핑은 구현 범위에 포함되지 않습니다.
- IPv6와 VLAN·PPPoE 등 추가 헤더가 있는 패킷은 현재 eBPF 파서의 분류 대상에 포함되지 않습니다.
- 플로우 분류는 통계 기반 추정이므로 실제 애플리케이션의 성격과 다를 수 있습니다.
- 보호 동작은 실시간·벌크 플로우의 동시 관측을 기준으로 합니다. 지연 시간이나 패킷 손실률을 직접 피드백하는 제어는 구현되어 있지 않습니다.
- 상태 로그는 분류된 플로우 수, 보호 상태와 데몬이 추적하는 벌크 속도를 보여줍니다. 실제 TC 적용 상태는 `show_tc_state.sh`로 확인할 수 있습니다.
